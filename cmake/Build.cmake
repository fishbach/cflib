...
# library
function(cf_lib lib)
    cmake_parse_arguments(ARG "ENABLE_MOC;ENABLE_SER;ENABLE_UIC" "PCH" "PUBLIC;PRIVATE;DIRS;OTHER_FILES" ${ARGN})

    cf_find_sources(sources . ${ARG_DIRS} OTHER_FILES ${ARG_OTHER_FILES})
    add_library(${lib} ${sources})
    cf_configure_target(${lib} ${ARG_ENABLE_MOC} ${ARG_ENABLE_SER} ${ARG_ENABLE_UIC} "${ARG_PCH}")
...
# application
function(cf_app app)
    cmake_parse_arguments(ARG "ENABLE_MOC;ENABLE_SER;ENABLE_UIC;ENABLE_GIT_VERSION;CF_INTERN" "PCH" "DIRS;OTHER_FILES" ${ARGN})

    cf_find_sources(sources . ${ARG_DIRS} OTHER_FILES ${ARG_OTHER_FILES})
    add_executable(${app} ${sources})
    cf_configure_target(${app} ${ARG_ENABLE_MOC} ${ARG_ENABLE_SER} ${ARG_ENABLE_UIC} "${ARG_PCH}")
...
function(cf_test test lib)
    ...
    cf_configure_target(${test} TRUE ${ARG_ENABLE_SER} FALSE "${ARG_PCH}")
    target_link_libraries(${test} PRIVATE ${lib} Qt5::Test)
    add_test(NAME ${test} COMMAND ${test})
endfunction()

# configure target
function(cf_configure_target target enable_moc enable_ser enable_uic pch)
    # PCH
    if(ENABLE_PCH)
        if(pch)
