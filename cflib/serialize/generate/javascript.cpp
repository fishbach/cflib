...
String pathPrefix(const String & fromPath, const String & toPath)
{
    StringList from = fromPath.split('/');
...
{
    const String nsPath = ti.getNSPath();
    String js = header();
    js << "import __ber from '" << pathPrefix(nsPath, "cflib/net") << "ber.js';\n";
    if (ti.isRMIService()) js << "import __rmi from '" << pathPrefix(nsPath, "cflib/net") << "rmi.js';\n";
    else                   js << "import __inherit from '" << pathPrefix(nsPath, "cflib/util") << "inherit.js';\n";
    if (!ti.cfSignals.isEmpty()) js << "import __RSig from '" << pathPrefix(nsPath, "cflib/net") << "rsig.js';\n";
    for (const SerializeTypeInfo & mTi : getMemberTypes(ti)) {
        String name = mTi.getName();
        name.replace("::", "__");
        js << "import " << name;
        js << " from '" << pathPrefix(nsPath, mTi.getNSPath()) << mTi.typeName.toLower() << ".js';\n";
    }
    js << "\n";
...
String generatePermissions(const String & nsPath, const List<Permission> & permissions)
{
    String js = header();
    js <<
        "import __permission from '" << pathPrefix(nsPath, "cflib/util") << "permission.js';\n"
        "\n";
    ...
    if (!nsPath.isEmpty()) {
        js << "\n"
            "export default " << String(nsPath).replace("/", ".") << ";\n";
    }
...
void generateJavaScript(const StructuredTypeInfos & typeInfos, const String & dest)
{
    ...
    // write permissions
    if (!typeInfos.permissions.isEmpty()) {
        Set<String> paths;
        for (const Permission & perm : typeInfos.permissions) {
            paths << perm.getNSPath();
        }
        for (const String & nsPath : paths) {
            util::mkPath(destJs + nsPath);
            String path = (nsPath.isEmpty() ? "" : (nsPath + "/")) + "permissions.js";
            files << path;
            String js = generatePermissions(nsPath, typeInfos.permissions);
            File::write(destJs + path, js.toUtf8());
        }
    }
