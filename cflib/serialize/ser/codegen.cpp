...
{
    std::string fullName = (cl.ns + "::" + cl.typeName).toStdString();
...
void writeMethods(std::ostream & out, const SerializeTypeInfo & cl)
{
    if (cl.hasBase()) {
        out << std::format(
            ...
            "\n",
            cl.typeName.toStdString(), cl.typeName.toStdString());
    }
    ...
    if (cl.members.isEmpty() && !cl.hasBase()) {
        ...
        out << std::format(
            ...
            "    ser << ",
            cl.typeName.toStdString());
        if (cl.hasBase()) {
            out << std::format("(uint32){} << (const {} &)*this",
                calcClassHash(cl), cl.getBase().typeName.toStdString());
        } else {
        ...
        for (const SerializeVariableTypeInfo & m : cl.members) {
            ...
            else out << std::format(" << {}", m.name.toStdString());
        }
        out << std::format(
            ";\n"
            "}}\n"
            ...
            cl.typeName.toStdString());
        if (cl.hasBase()) {
            out << std::format(" >> ({} &)*this", cl.getBase().typeName.toStdString());
        }
        for (const SerializeVariableTypeInfo & m : cl.members) {
            if (m.name.isEmpty()) out << " >> cflib::serialize::Placeholder()";
            else out << std::format(" >> {}", m.name.toStdString());
    ...
    for (const SerializeFunctionTypeInfo & f : list) {
        ...
        if (!f.parameters.isEmpty()) {
            ...
            for (const SerializeVariableTypeInfo & p : f.parameters) {
                out << std::format("            {} ", p.type.typeName.toStdString());
                if (p.name.isEmpty()) out << std::format("__param_{}", ++id);
                else out << std::format("__{}", p.name.toStdString());
                out << ";\n";
            }
            out << "            __deser";
            id = 0;
        ...
        out << std::format("{}(", f.name.toStdString());
        int id = 0;
        bool isFirst = true;
        for (const SerializeVariableTypeInfo & p : f.parameters) {
...
void writeFunction(const SerializeFunctionTypeInfo & f, std::ostream & out)
{
    ...
    if (!f.returnType.isNull()) {
        out << std::format("        func.returnType = cflib::serialize::impl::fromType<{}>();\n",
            f.returnType.typeName.toStdString());
    }
    if (!f.parameters.isEmpty()) {
        out << "        func.parameters";
        for (const SerializeVariableTypeInfo & m : f.parameters) {
            out << std::format(
                ...
                m.name.toStdString(), m.type.typeName.toStdString(), m.isRef ? "true" : "false");
        }
        out << ";\n";
    }
...
int genSerialize(const std::string & headerName, const HeaderParser & hp, std::ostream & out)
{
    ...
    if (!hp.permissions().isEmpty()) {
        out << "\n";
        for (const Permission & perm : hp.permissions()) {
            String var = perm.name;
            var.replace(".", "::");
            out << "cflib::serialize::Permission " << var.str() << "(\"" << perm.name.str() << "\"";
    ...
    for (const SerializeTypeInfo & cl : hp.classes()) {
        ...
        out << "\n";
        std::vector<std::string> nsList = splitStr(cl.ns.toStdString(), "::");
        bool isFirst = true;
        ...
        out << std::format(
            "    retval.ns = \"{}\";\n"
            "    retval.typeName = \"{}\";\n",
            cl.ns.toStdString(), cl.typeName.toStdString());
        if (cl.hasBase()) {
            out << std::format("    retval.bases.push_back(cflib::serialize::impl::fromType<{}>());\n",
                cl.getBase().typeName.toStdString());
        }
        if (!cl.members.isEmpty()) {
            ...
            for (const SerializeVariableTypeInfo & m : cl.members) {
                if (m.name.isEmpty()) {
                    ...
                    out << std::format(
                        "\n        << cflib::serialize::SerializeVariableTypeInfo(\"{}\", cflib::serialize::impl::fromType<{}>())",
                        m.name.toStdString(), m.type.typeName.toStdString());
                }
            }
            out << ";\n";
        }
        ...
        if (cl.isRMIService()) {
            ...
            if (existsWithoutReturnValues) {
                out << std::format("void {}::processRMIServiceCallImpl(cflib::serialize::BERDeserializer &{}, uint __callNo) {{\n",
                    cl.typeName.toStdString(), existsWithoutReturnValuesAndParameters ? " __deser" : "");
            ...
            if (existsWithReturnValues) {
                out << std::format("void {}::processRMIServiceCallImpl(cflib::serialize::BERDeserializer &{}, uint __callNo, cflib::serialize::BERSerializer & __ser) {{\n",
                    cl.typeName.toStdString(), existsWithReturnValuesAndParameters ? " __deser" : "");
