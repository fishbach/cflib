...
// ============================================================
// Free function wrappers (backward-compatible delegates)
// ============================================================

template<typename T>
inline List<T> setValues(const Set<T>& s) { return s.values(); }

inline ByteArray join(const List<ByteArray>& list, char sep) { return list.join(sep); }
inline String    join(const List<String>&    list, char sep) { return list.join(sep); }

template<typename T>
inline void sort(List<T>& v) { v.sort(); }

template<typename K, typename V>
inline List<K> keys(const Map<K, V>& m) { return m.keys(); }

template<typename K, typename V>
inline V mapValue(const Map<K, V>& m, const K& key, const V& def = V()) { return m.value(key, def); }

template<typename K, typename V>
inline V hashValue(const Hash<K, V>& m, const K& key, const V& def = V()) { return m.value(key, def); }

template<typename K, typename V>
inline bool contains(const Map<K, V>& m, const K& key) { return m.contains(key); }

template<typename T>
inline bool contains(const Set<T>& s, const T& val) { return s.contains(val); }

template<typename T, typename U>
inline bool contains(const List<T>& v, const U& val) { return v.contains(val); }

template<typename T>
inline void remove(Set<T>& s, const T& val) { s.remove(val); }

template<typename T>
inline T takeFirst(List<T>& v) { return v.takeFirst(); }

template<typename T>
inline T takeLast(List<T>& v) { return v.takeLast(); }

template<typename T>
inline const T& last(const List<T>& v) { return v.last(); }

template<typename T>
inline const T& last(const std::vector<T>& v) { return v.back(); }
